...
function approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile "$ORDERER_CA" --channelID mychannel --name ${CC_NAME} --version 1.0 --package-id ${PACKAGE_ID} --sequence 1 ${INIT_REQUIRED} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
function checkCommitReadiness() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ${CC_NAME} --version 1.0 --sequence 1 ${INIT_REQUIRED} --output json >&log.txt
...
function commitChaincodeDefinition() {
  ...
  # it using the "-o" option
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version 1.0 --sequence 1 ${INIT_REQUIRED} >&log.txt
